...
def train(train_exs, test_exs, word_embeddings):
    print "HEY"
    ...
    trainQ1_mat = np.asarray([pad_to_length(np.array(ex.indexed_q1), seq_max_len) for ex in train_exs])
    trainQ2_mat = np.asarray([pad_to_length(np.array(ex.indexed_q2), seq_max_len) for ex in train_exs])
    trainQ1_seq_lens = np.array([len(ex.indexed_q1) for ex in train_exs])
    trainQ2_seq_lens = np.array([len(ex.indexed_q2) for ex in train_exs])
    ...
    q1_len = tf.placeholder(tf.int32, 1)
    q2_len = tf.placeholder(tf.int32, 1)
    ...
    sent_input = sent
    output1, _ = tf.nn.dynamic_rnn(myLSTMcell1, sent_input, dtype=tf.float32) #sequence_length=sent_len, dtype=tf.float32)
    output2, _ = tf.nn.dynamic_rnn(myLSTMcell2, sent_input, dtype=tf.float32) #sequence_length=sent_len, dtype=tf.float32)
    ...
    z1 = tf.reduce_mean(output1[0], axis=0)
    z2 = tf.reduce_mean(output2[0], axis=0)
